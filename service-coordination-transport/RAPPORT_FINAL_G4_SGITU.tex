...
G7 & Suivi véhicules & Flux positions GPS & Kafka \\
...
\textbf{AffectationVehiculeLigne} : association temporelle véhicule $\leftrightarrow$ ligne.\\
\textbf{Mission} : unité centrale d'exploitation (\texttt{vehiculeId}, \texttt{chauffeurId}, \texttt{statut} : PLANIFIEE, EN\_COURS, CLOTUREE, ANNULEE).\\
...
\begin{itemize}[leftmargin=*]
    ...
    \item Compose local G4 autonome : \texttt{docker compose up -d --build} ;
    \item Intégration monorepo : le groupe G4 ajoute \textbf{uniquement} \texttt{g4-coordination} + \texttt{g4-postgres} dans le compose racine (pas de stack G5/G7 imposée par G4) ;
    \item Variables centralisées : \texttt{.env.example} (bloc G4 seulement côté promo).
\end{itemize}
...
\section{Modèle de sécurité retenu}
G4 applique une défense en profondeur :
\begin{enumerate}[leftmargin=*]
    \item \textbf{Authentification} : JWT signé (secret partagé promo \texttt{JWT\_SECRET}) ;
    \item \textbf{Autorisation} : rôles vérifiés localement via \texttt{@PreAuthorize} ;
    \item \textbf{Validation métier} : conducteur validé chez G3 si activé ;
